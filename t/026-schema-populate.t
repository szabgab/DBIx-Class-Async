...
sub reset_data {
    $schema->resultset('Order')->delete->get;
    $schema->resultset('User')->delete->get;
}
...
subtest 'populate - hashref format via resultset' => sub {
    ...
    my $dave = $schema->resultset('User')->find({ name => 'Dave' })->get;
...
subtest 'populate - many records' => sub {
    ...
    my $users = $schema->populate('User', \@data)->get;
    ...
    my $active_count = $schema->resultset('User')->search({ active => 1 })->count->get;
...
subtest 'populate - with relationships' => sub {
    ...
    my $orders = $schema->populate('Order', [
    ...
    my $alice = $schema->resultset('User')->find($alice_id)->get;
    my $alice_orders = $alice->orders->count->get;
...
subtest 'populate - returns Future' => sub {
    ...
    my $future = $schema->populate('User', [
...
subtest 'populate - error: not an arrayref' => sub {
    ...
    eval {
        $schema->populate('User', { name => 'Test' })->get;
    };
...
subtest 'populate - error: missing source name' => sub {
    ...
    eval {
        $schema->populate(undef, [{ name => 'Test' }])->get;
    };
...
subtest 'populate - mixed with manual creates' => sub {
    ...
    my $total = $schema->resultset('User')->count->get;
    is($total, 3, 'Total of 3 users (1 manual + 2 populate)');

    my $manual = $schema->resultset('User')->find({ name => 'Manual' })->get;
    ok($manual, 'Manual user exists');

    my $pop1 = $schema->resultset('User')->find({ name => 'Pop1' })->get;
...
subtest 'populate - verify all records inserted' => sub {
    ...
    is($schema->resultset('User')->count->get, 3, 'Database has 3 records');

    ok($schema->resultset('User')->find({ name => 'Test1' })->get, 'Test1 exists');
    ok($schema->resultset('User')->find({ name => 'Test2' })->get, 'Test2 exists');
};
